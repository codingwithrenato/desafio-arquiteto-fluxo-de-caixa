...
| Logs estruturados | **Serilog** (console) correlacionados ao trace: cada log carrega `trace_id`/`span_id` da Activity atual (sink central em produção: ELK/App Insights). |
...
| Próximos passos | Métricas Prometheus + dashboards Grafana + UI unificada de logs (Loki/ELK) — ver [`future.md`](future.md). |
